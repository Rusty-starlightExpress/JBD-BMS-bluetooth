...
print("Run jbdbms-4-socket...")
child = pexpect.spawn("sudo /usr/bin/python3 /home/pi/jbdbms-4-socket-2temps.py -b %s -i 5 -m %s" % (DEVICE,BMSNAME))
...
while True:
  ...
  if r1t and r2t and r3t :
    msgstr = str(r'{"device": "%s","voltage": %.2f,"amp": %.2f,"watt": %.2f,"remain": %d,"capacity": %.2f,"cycles": %d,"cellmin": %.3f,"cellmax": %.3f,"delta": %d,"cell1": %.3f,"cell2": %.3f,"cell3": %.3f,"cell4": %.3f,"datetime": "%s"}' % (meter,volts,amps,watts,remain,capa,cycles,cmin,cmax,delta,c1,c2,c3,c4,dt))

  if r1t and r2t and r3t == False :
    msgstr = str(r'{"device": "%s","cellmin": %.3f,"cellmax": %.3f,"delta": %d,"cell1": %.3f,"cell2": %.3f,"cell3": %.3f,"datetime": "%s"}' % (meter,cmin,cmax,delta,c1,c2,c3,c4,dt))
